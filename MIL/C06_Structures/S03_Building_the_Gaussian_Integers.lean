...
OMIT. -/
/- TEXT:
もし ``instance : CommRing GaussInt := _`` と入力してvscodeに表示される電球マークをクリックし，Leanに構造体定義のスケルトンを埋めてもらうと，おびただしい数の項目が表示されます．しかし，構造体の定義に飛ぶと，多くのフィールドにはデフォルトの定義があり，これらはLeanが自動的に埋めてくれます．重要なものは以下の定義にあります． ``nsmul`` と ``zsmul`` は特殊なものですが，これは次章で説明するため現時点では無視してください．それぞれの場合に置いて，関連する恒等式は定義を展開し， ``ext`` を使って恒等式を実部と虚部に戻し，単純化し，必要であれば整数で関連する環の計算を行うことで証明されます．このコードをすべて繰り返さないようにすることは簡単ですが，これは現在の議論のテーマではないことに注意してください．
BOTH: -/
-- QUOTE:
...
OMIT. -/
/- TEXT:
実部と虚部は整数ではないかもしれませんが，最も近い整数 :math:`u` と :math:`v` に丸めることが出来ます．丸めた際の余った部分を :math:`u'+v'i` として，右辺を :math:`(u + vi) + (u' + v'i)` と表現できます．ここで :math:`|u'| \le 1/2` と :math:`|v'| \le 1/2` という条件があることに注意すると以下のようになります．
