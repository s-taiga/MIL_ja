...
OMIT. -/
/- TEXT:
上記の定理の中で， ``exp_le_exp`` と ``exp_lt_exp`` は必要十分条件を意味する **双方向の含意** （bi-implication）を用いています．（VSCodeでは ``\lr`` か ``\iff`` で入力することができます）．この論理結合子については次の章で詳しく説明することとします．このような定理は ``rw`` と一緒に使うことで，ゴールをそれと等価なものに書き換えることができます:
TEXT. -/
-- QUOTE:
